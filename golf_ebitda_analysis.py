...
# 민감도 분석 범위 설정
# 객단가: 9만원 ~ 13만원 (1만원 단위)
price_per_customer = np.arange(90000, 140000, 10000)  # 90k, 100k, 110k, 120k, 130k

# 내장객 수: 12만명 ~ 16만명 (1만명 단위)
visitors = np.arange(120000, 170000, 10000)  # 120k, 130k, 140k, 150k, 160k
...
ebitda_table = pd.DataFrame(
    index=[f'{v//10000}' for v in visitors],  # 만명 단위로 표시
    columns=[f'{p//10000}' for p in price_per_customer]  # 만원 단위로 표시
)
...
plt.title('Golf Course M&A - EBITDA Sensitivity Analysis\n(EBITDA in billion KRW)',
          fontsize=16, fontweight='bold', pad=20)
plt.xlabel('Price per Customer (10K KRW)', fontsize=12, fontweight='bold')
plt.ylabel('Annual Visitors (10K)', fontsize=12, fontweight='bold')
